...
# We don't necessarily need message_content for slash commands, 
# but the warning in logs suggests it might be expected by the library setup.
# However, for slash commands only, default is fine.
...
def run_flask():
    # Render and other platforms use PORT env var
    port = int(os.environ.get("PORT", 5000))
    app.run(host='0.0.0.0', port=port)

# Headers to mimic a browser, helping bypass basic bot detection or WAF issues
ROBLOX_HEADERS = {
...
async def altcheck(interaction: discord.Interaction, username: str):
    ...
    if userid is None:
        return await interaction.followup.send("User not found or Roblox API blocked the request.")
...
async def scan(interaction: discord.Interaction, username: str):
    ...
    embed = discord.Embed(title=f"Deep Scan Report: {userinfo['name']}", color=risk_color(risk_level))
...
async def export(interaction: discord.Interaction, count: int = 10):
    ...
    if len(json_data) > 1900:
        ...
        with open(filename, 'w') as f:
            f.write(json_data)
        await interaction.followup.send(f"Exported {len(export_data)} scan(s):", file=discord.File(filename), ephemeral=True)
        os.remove(filename)
    else:
